...
export const fileValidator = mixed()
  .test('file size', 'Fichier trop lourd. Il doit être inférieur à 2Mo', value => {
    return value && value.size ? value.size <= MAX_FILE_SIZE : true
  })
  .test('file type', 'Format invalide. Seul les jpg, png et pdf sont acceptés', value => {
    return value ? SUPPORTED_FORMATS.includes(value.type.toLowerCase()) : true
  })
